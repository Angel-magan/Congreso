...
const TrabajoCard = ({ titulo, fecha_hora, sala, chairman, ponente }) => {
  ...
  return (
    <div className="border border-3 border-warning rounded p-3 trabajo-card d-flex flex-column">
      ...
      <p className="mt-2"><strong>Sala:</strong> {sala}</p>
      <p><i className="bi bi-mic-fill text-primary"></i> <strong>Chairman:</strong> {chairman}</p>
      <p><i className="bi bi-megaphone-fill text-danger"></i> <strong>Ponente:</strong> {ponente}</p>
    </div>
  );
};

const MisSesiones = ({ rol }) => {
    ...
    const fetchTrabajos = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/sesiones/getSesionesPorUsuario/${congresistaData.id_congresista}/${rol}`
        );
  ...
  return (
    <div className="row mt-3" style={{ maxHeight: "700px", overflowY: "auto", border: "3px solid #00BFFF", borderRadius: "10px", padding: "15px" }}>
      ...
      {trabajos.length > 0 ? (
        ...
        <p>No tienes sesiones como {rol}.</p>
      )}
    </div>
  );
};
