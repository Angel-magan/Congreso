...
const ReporteCongresistas = () => {
    ...
    const fetchCongresistas = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/congresistas/getCongresistas");
    ...
    const generatePDF = (data) => {
        ...
        doc.setFontSize(18);
        doc.text("Reporte de Congresistas Registrados", 50, 20);
        doc.setFontSize(13);
        const description = "Descripción: Reporte que muestra la lista de congresistas registrados.";
        ...
        doc.autoTable({
            startY: 60,
            head: [["ID", "Nombre", "Apellido", "Institución", "Teléfono"]],
            body: data.map((item) => [
                item.id_congresista,
                item.nombre,
                item.apellido,
    ...
    const downloadPDF = () => {
        if (pdfBlobUrl) {
            ...
            link.download = "Reporte_Congresistas.pdf";
            link.click();
        }
    };
    ...
    return (
        <div>
            <button
                ...
                onClick={fetchCongresistas}
            >
                Reporte de Congresistas
            </button>
...
export default ReporteCongresistas;