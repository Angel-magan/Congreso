...
const ReporteTrabajosNoAceptados = () => {
    const [pdfBlobUrl, setPdfBlobUrl] = useState(null);
    const [noDataMessage, setNoDataMessage] = useState(""); // Estado para manejar el mensaje de falta de datos

    const fetchTrabajosNoAceptados = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/users/getTrabajosNoAceptados");
            ...
            console.error("Error fetching trabajos no aceptados:", error);
    ...
    const generatePDF = (data) => {
        ...
        doc.setFontSize(18);
        doc.text("Reporte de Trabajos No Aceptados", 50, 20);
        doc.setFontSize(13);
        const description = "Descripción: Reporte de los trabajos que no fueron aceptados por los miembros del comité.";
    ...
    const downloadPDF = () => {
        if (pdfBlobUrl) {
            ...
            link.download = "Reporte_Trabajos_No_Aceptados.pdf";
            link.click();
        }
    };
    ...
    return (
        <div>
            <button
                ...
                onClick={fetchTrabajosNoAceptados}
            >
                Reporte de Trabajos No Aceptados
            </button>
...
export default ReporteTrabajosNoAceptados;