...
const Card = ({ titleJob, date, clock, hall, chairman, ponentesTrabajo, idSesion }) => {
  ...
  const handleAsistir = async () => {
	...
	try {
		const response = await axios.post("http://localhost:5000/api/sesiones/asistir-sesion", {
		...
		<div className="card border-warning w-100 me-3">
			...
			{isCongresista && !yaAsisto && (
				<div className="d-flex justify-content-center mb-3">
					<button className="btn bg-warning text-white w-50"
					onClick={handleAsistir}>Asistir</button>
				</div>
			)}
